<template>
    <router-link :to="{ name: 'mentorat.mentor', params: { id: mentor.id } }"
        class="block bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden hover:border-violet-200 hover:shadow-md transition group">
        <!-- Header gradient -->
        <div class="h-20 bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-500 relative">
            <div class="absolute -bottom-8 left-5">
                <div class="w-16 h-16 rounded-full bg-white dark:bg-slate-800 border-4 border-white shadow-sm flex items-center justify-center text-2xl font-black text-violet-600">
                    {{ initials }}
                </div>
            </div>
        ...
        <div class="pt-10 px-5 pb-5">
            <h3 class="font-bold text-lg text-slate-900 dark:text-slate-100 group-hover:text-violet-700 transition line-clamp-1">
                {{ mentor.name }}
            </h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ mentor.country }}{{ mentor.city ? ', ' + mentor.city : '' }}</p>
            <p v-if="mentor.bio" class="text-sm text-slate-600 dark:text-slate-300 mt-2 line-clamp-2">{{ mentor.bio }}</p>
            ...
            <div v-if="mentor.skills?.length" class="flex flex-wrap gap-1 mt-3">
                ...
                <span v-if="mentor.skills.length > 4" class="text-[10px] text-slate-400 dark:text-slate-500 self-center">
            ...
            <div class="flex items-center gap-4 mt-4 text-xs text-slate-500 dark:text-slate-400">
                <span v-if="mentor.avg_rating > 0" class="flex items-center gap-1">
                    ...
                    <span class="text-slate-400 dark:text-slate-500">({{ mentor.reviews_count }})</span>
                </span>
            ...
            <div v-else class="mt-3 flex items-center gap-1.5">
                ...
                <span class="text-xs text-slate-400 dark:text-slate-500">Créneaux non renseignés</span>
            </div>
        </div>
    </router-link>
</template>
...
function levelClass(level) {
    ...
    }[level] || 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300';
}
</script>
