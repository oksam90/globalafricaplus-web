<template>
    <div v-if="loading" class="max-w-7xl mx-auto p-12 text-slate-500">Chargement…</div>
    <section v-else-if="sector" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <router-link to="/secteurs" class="text-sm text-emerald-700 hover:underline">← Tous les secteurs</router-link>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            <div class="bg-white border border-slate-100 rounded-2xl p-5 text-center">
                <div class="text-3xl font-black text-slate-900">{{ stats.projects_count }}</div>
                <div class="text-xs text-slate-500 uppercase tracking-wider mt-1">Projets</div>
            </div>
            <div class="bg-white border border-slate-100 rounded-2xl p-5 text-center">
                <div class="text-3xl font-black text-slate-900">{{ stats.countries_count }}</div>
                <div class="text-xs text-slate-500 uppercase tracking-wider mt-1">Pays</div>
            </div>
            <div class="bg-white border border-slate-100 rounded-2xl p-5 text-center">
                <div class="text-3xl font-black text-emerald-700">{{ formatShort(stats.total_raised) }}</div>
                <div class="text-xs text-slate-500 uppercase tracking-wider mt-1">Levés</div>
            </div>
            <div class="bg-white border border-slate-100 rounded-2xl p-5 text-center">
                <div class="text-3xl font-black text-slate-900">{{ stats.jobs_target }}</div>
                <div class="text-xs text-slate-500 uppercase tracking-wider mt-1">Emplois visés</div>
            </div>
        </div>
        ...
        <div v-if="sector.sub_categories?.length" class="mb-10">
            ...
            <div class="flex flex-wrap gap-2">
                <router-link v-for="sub in sector.sub_categories" :key="sub.id"
                    ...
                    class="px-3 py-1.5 rounded-full text-sm font-medium bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 transition">
                    {{ sub.name }}
                </router-link>
            </div>
        </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-5">
                ...
                <router-link :to="{ name: 'projects.index', query: { category: sector.id } }"
                    class="text-sm text-emerald-700 hover:underline font-semibold">
                    Voir tous les projets →
                </router-link>
            </div>

            <div v-if="!topProjects.length" class="text-center py-10 text-slate-500">
        ...
        <div v-if="stats.total_needed > 0" class="mt-10 bg-white border border-slate-100 rounded-2xl p-6">
            ...
            <div class="mt-1 text-xs text-slate-500 text-right">
                sur {{ formatShort(stats.total_needed) }} recherchés
            </div>
        </div>
    </section>
    <div v-else class="max-w-7xl mx-auto p-12 text-slate-500">Secteur introuvable.</div>
</template>
