<template>
    <div v-if="loading" class="max-w-7xl mx-auto px-4 py-16 text-slate-500">Chargement du profil…</div>
    <div v-else-if="!mentor" class="max-w-7xl mx-auto px-4 py-16 text-slate-500">Mentor introuvable.</div>
    <div v-else>
        <!-- Header -->
        <section class="bg-gradient-to-br from-violet-50 via-fuchsia-50 to-slate-50 border-b border-slate-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div class="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-slate-500">{{ mentor.country }}{{ mentor.city ? ', ' + mentor.city : '' }}</p>
                        <p v-if="mentor.bio" class="mt-3 text-slate-700 max-w-2xl">{{ mentor.bio }}</p>
                        ...
                        <div class="flex flex-wrap gap-5 mt-4">
                            <div v-if="mentor.avg_rating > 0" class="flex items-center gap-1.5">
                                ...
                                <span class="text-sm font-semibold text-slate-700">{{ parseFloat(mentor.avg_rating).toFixed(1) }}</span>
                                <span class="text-sm text-slate-500">({{ mentor.reviews_count }} avis)</span>
                            </div>
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="grid md:grid-cols-2 gap-3">
                            <div v-for="s in mentor.skills" :key="s.id"
                                ...
                                <div class="text-right">
                                    ...
                                    <div v-if="s.pivot?.years_experience" class="text-[10px] text-slate-400 mt-1">
                    ...
                    <div>
                        ...
                        <div v-if="reviews.length === 0" class="text-slate-500 py-6 text-center bg-slate-50 rounded-xl">
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] text-slate-400">{{ formatDate(r.created_at) }}</div>
                                        </div>
                                    </div>
                                    <div class="flex gap-0.5">
                    ...
                    <div v-if="mentorProfile" class="bg-violet-50 rounded-2xl p-5">
                        ...
                        <div class="space-y-2 text-sm">
                            <div v-if="mentorProfile.data?.expertise">
                                <span class="text-slate-500">Expertise :</span>
                            ...
                            <div v-if="mentorProfile.data?.languages?.length">
                                <span class="text-slate-500">Langues :</span>
                    ...
                    <div class="bg-white border border-slate-100 rounded-2xl p-5">
                        ...
                        <div v-if="mentor.mentor_availabilities?.length" class="space-y-2">
                            <div v-for="a in mentor.mentor_availabilities" :key="a.id"
                                ...
                                <span class="text-slate-500">{{ a.start_time.slice(0, 5) }} — {{ a.end_time.slice(0, 5) }}</span>
                            </div>
                        </div>
                        <p v-else class="text-sm text-slate-400">Créneaux non renseignés.</p>
                    </div>

                    <!-- Member since -->
                    <div class="bg-slate-50 rounded-2xl p-5 text-sm text-slate-600">
        ...
        <div v-if="showRequestModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="showRequestModal = false">
            <div class="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="showRequestModal = false" class="text-slate-400 hover:text-slate-600 text-xl">&times;</button>
                </div>
                ...
                <form @submit.prevent="submitRequest" class="space-y-4">
                    <div>
                        ...
                        <input v-model="requestForm.topic" type="text" required maxlength="200" placeholder="Ex: Stratégie de levée de fonds"
                            class="w-full px-3 py-2 rounded-md border border-slate-200 focus:border-violet-400 focus:outline-none text-sm" />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea v-model="requestForm.message" rows="3" maxlength="2000"
                            ...
                            class="w-full px-3 py-2 rounded-md border border-slate-200 focus:border-violet-400 focus:outline-none text-sm"></textarea>
                    </div>
...
function levelClass(l) {
    return { expert: 'bg-violet-100 text-violet-700', advanced: 'bg-blue-50 text-blue-700', intermediate: 'bg-emerald-50 text-emerald-700', beginner: 'bg-slate-100 text-slate-600' }[l] || 'bg-slate-100 text-slate-600';
}
