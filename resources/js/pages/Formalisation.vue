<template>
    <div>
        <!-- Hero -->
        <section class="bg-gradient-to-br from-emerald-50 via-teal-50 to-green-50 dark:from-emerald-950/40 dark:via-teal-950/30 dark:to-green-950/40 py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-10 max-w-2xl">
                    <div class="text-center">
                        ...
                        <div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mt-1">Pays couverts</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-black text-emerald-700">{{ stats.templates || 0 }}</div>
                        <div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mt-1">Templates BP</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-black text-emerald-700">{{ stats.partners || 0 }}</div>
                        <div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mt-1">Partenaires microfinance</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-black text-emerald-700">{{ stats.users_formalized || 0 }}</div>
                        <div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mt-1">Entrepreneurs accompagnés</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div v-else class="grid md:grid-cols-3 gap-6">
                <div v-for="c in countries" :key="c.country"
                    ...
                    class="bg-white dark:bg-slate-800 border rounded-2xl p-6 cursor-pointer hover:border-emerald-300 transition"
                    :class="selectedCountry === c.country ? 'border-emerald-500 ring-2 ring-emerald-200' : 'border-slate-100 dark:border-slate-700'">
                    <h3 class="text-lg font-bold">{{ c.country }}</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ c.steps_count }} étapes de formalisation</p>
                    ...
                    <div v-for="step in steps" :key="step.id"
                        ...
                        <div v-if="openSteps.has(step.id)" class="border-t border-slate-100 dark:border-slate-700 p-5 bg-slate-50/50">
                            ...
                            <div v-if="step.required_documents?.length" class="mt-4">
                                ...
                                <ul class="space-y-1">
                                    <li v-for="(doc, i) in step.required_documents" :key="i" class="text-sm text-slate-600 dark:text-slate-300 flex items-center gap-2">
        ...
        <section class="bg-slate-50 dark:bg-slate-900 py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <p class="text-slate-600 dark:text-slate-300 mb-6">Accédez au financement adapté à votre stade de développement.</p>
                <div v-if="partners.length" class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div v-for="p in filteredPartners" :key="p.id" class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-5">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <h3 class="font-bold">{{ p.name }}</h3>
                                <span class="text-xs text-slate-500 dark:text-slate-400">{{ p.country }} · {{ p.type }}</span>
                            </div>
                        </div>
                        <p v-if="p.description" class="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">{{ p.description }}</p>
        ...
        <section class="bg-emerald-700 py-12">
            <div class="max-w-3xl mx-auto px-4 text-center text-white">
                ...
                <div class="flex justify-center gap-3 mt-6">
                    <router-link to="/inscription" class="px-6 py-3 rounded-md bg-white dark:bg-slate-800 text-emerald-700 font-semibold hover:bg-emerald-50">
                        Créer mon compte
                    </router-link>
