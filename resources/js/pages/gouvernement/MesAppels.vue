    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex items-end justify-between mb-8">
            <div>
                ...
                <p class="text-slate-600 mt-1">Créez, gérez et suivez vos appels à projets publics.</p>
            </div>
            <div class="flex gap-2">
                <router-link to="/gouvernement" class="text-sm font-semibold text-sky-600 hover:underline">
                    Portail public →
                </router-link>
        ...
        <div v-else-if="filteredCalls.length === 0" class="text-center py-16 text-slate-500 bg-slate-50 rounded-2xl">
            Aucun appel {{ currentTab === 'all' ? '' : 'avec ce statut' }}.
            <button @click="showCreateModal = true" class="block mx-auto mt-4 text-sky-600 font-semibold hover:underline">
        ...
        <div v-else class="space-y-4">
            <div v-for="c in filteredCalls" :key="c.id"
                ...
                :class="c.status === 'draft' ? 'border-amber-200' : 'border-slate-100'">
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span v-if="c.sector" class="text-xs text-slate-500">{{ c.sector }}</span>
                        </div>
                        <router-link :to="{ name: 'gouvernement.call', params: { slug: c.slug } }"
                            class="text-lg font-bold hover:text-sky-700">{{ c.title }}</router-link>
                    ...
                    <div class="flex gap-2 shrink-0">
                        ...
                        <router-link v-if="c.applications_count > 0"
                            ...
                            class="px-3 py-1.5 text-sm font-semibold rounded-md border border-sky-200 text-sky-700 hover:bg-sky-50">
                            Candidatures
                        </router-link>
                ...
                <form @submit.prevent="saveCall" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" rows="4" required maxlength="10000"
                            class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm"></textarea>
                    </div>
                    <div class="grid sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input v-model="form.geographic_zone" type="text" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                    </div>
                    <div class="grid sm:grid-cols-3 gap-3">
                        <div>
                            <label class="block text-sm font-medium mb-1">Secteur</label>
                            <input v-model="form.sector" type="text" placeholder="Agritech, Fintech…" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Budget</label>
                            <input v-model.number="form.budget" type="number" min="0" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Devise</label>
                            <select v-model="form.currency" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm">
                    ...
                    <div class="grid sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input v-model="form.opens_at" type="date" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Date limite</label>
                            <input v-model="form.closes_at" type="date" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" @click="closeModal"
                            class="px-5 py-2.5 rounded-md border border-slate-200 hover:bg-slate-50 text-sm font-semibold">
                            Annuler
                        </button>
                    </div>
                </form>
