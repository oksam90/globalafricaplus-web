<template>
    <router-link :to="{ name: 'projects.show', params: { slug: project.slug } }"
        class="block bg-white dark:bg-brand-black-50 rounded-2xl border border-brand-gold-200/50 dark:border-brand-gold-700/20 overflow-hidden hover:border-brand-gold-400 dark:hover:border-brand-gold-500/60 hover:shadow-md transition group">
        <div class="h-32 bg-gradient-to-br relative" :style="`background-image: linear-gradient(135deg, ${color}, #0f172a)`">
            <span class="absolute top-3 left-3 text-xs font-semibold px-2 py-1 rounded-md bg-white/90 text-slate-800">
            ...
            <span v-if="project.stage"
                class="absolute bottom-3 left-3 text-[10px] font-bold uppercase px-2 py-1 rounded bg-white/90 text-slate-700">
                {{ stageLabel }}
            </span>
        </div>
        <div class="p-5">
            <h3 class="font-bold text-slate-900 dark:text-white group-hover:text-brand-gold-700 dark:group-hover:text-brand-gold-400 transition line-clamp-1">
                {{ project.title }}
            </h3>
            <p v-if="project.sub_category" class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ project.sub_category.name }}</p>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-1.5 line-clamp-2">{{ project.summary }}</p>
