    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <router-link v-for="s in sectors" :key="s.id" :to="`/secteurs/${s.slug}`"
                class="group bg-white border border-slate-100 rounded-2xl p-6 hover:border-emerald-200 hover:shadow-md transition">
                <div class="flex items-start justify-between">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white font-black text-lg"
                        :style="{ backgroundColor: s.color }">
                        {{ s.name.charAt(0) }}
                    </div>
                    <span class="text-2xl font-black text-slate-900">{{ s.projects_count }}</span>
                </div>
                ...
                <div class="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
                    <div>
                        <div class="text-slate-400 uppercase tracking-wider text-[10px]">Levés</div>
                        <div class="font-semibold">{{ formatShort(s.total_raised) }}</div>
                    </div>
                    <div>
                        <div class="text-slate-400 uppercase tracking-wider text-[10px]">Emplois</div>
