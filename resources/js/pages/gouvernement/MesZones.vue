    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex items-end justify-between mb-8">
            <div>
                ...
                <p class="text-slate-600 mt-1">Gérez les ZES de votre territoire.</p>
            </div>
            <div class="flex gap-2">
                <router-link to="/gouvernement" class="text-sm font-semibold text-sky-600 hover:underline">Portail public →</router-link>
        ...
        <div v-else-if="zones.length === 0" class="text-center py-16 text-slate-500 bg-slate-50 rounded-2xl">
            Aucune zone économique enregistrée.
        </div>
        <div v-else class="grid md:grid-cols-2 gap-6">
            <div v-for="z in zones" :key="z.id" class="bg-white border border-slate-100 rounded-2xl p-6">
                ...
                <p v-if="z.description" class="text-sm text-slate-600 mt-1 line-clamp-2">{{ z.description }}</p>
                <div v-if="z.area_hectares" class="text-xs text-slate-500 mt-1">{{ z.area_hectares }} ha</div>
                <div v-if="z.sectors?.length" class="flex flex-wrap gap-1 mt-2">
                    <span v-for="s in z.sectors" :key="s" class="text-[10px] px-2 py-0.5 rounded-full bg-sky-50 text-sky-700 font-medium">{{ s }}</span>
                </div>
                <div v-if="z.incentives?.length" class="mt-2">
                    <div v-for="inc in z.incentives.slice(0,3)" :key="inc" class="text-xs text-emerald-700">✓ {{ inc }}</div>
                </div>
                <div class="flex gap-3 mt-4">
                    <button @click="openEdit(z)" class="text-xs font-semibold text-sky-600 hover:underline">Modifier</button>
                    <button @click="deleteZone(z.id)" class="text-xs font-semibold text-red-600 hover:underline">Supprimer</button>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-6">
                ...
                <form @submit.prevent="saveZone" class="space-y-4">
                    <div>
                        ...
                        <input v-model="form.name" type="text" required class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium mb-1">Pays *</label>
                            <input v-model="form.country" type="text" required class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Région</label>
                            <input v-model="form.region" type="text" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Description</label>
                        <textarea v-model="form.description" rows="3" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm"></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium mb-1">Superficie (ha)</label>
                            <input v-model.number="form.area_hectares" type="number" min="0" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Statut</label>
                            <select v-model="form.status" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm">
                    ...
                    <div>
                        ...
                        <input v-model="form.website" type="url" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Email contact</label>
                        <input v-model="form.contact_email" type="email" class="w-full px-3 py-2 rounded-md border border-slate-200 text-sm" />
                    </div>
